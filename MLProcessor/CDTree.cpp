...
CDTree::CDTree(int deepestTree, int max_bin, float thresholdInfoGain)
{
    ...
    one = new MyNode<double>("Yes", -1);
    zero = new MyNode<double>("No", -1);
}
...
int CDTree::buildTree(const MatrixXf &trainX, const MatrixXi &trainY, string Algorithm)
{
    ...
    // add by cr
    std::cout << "attribute num: " << this->intervals.size() << std::endl;

/*
    for(int i = 0; i < this->intervals.size(); i++)
    {
        vector<std::pair<float, float>> temp = this->intervals[i];
        qDebug() << temp;
    }
    */
...
int CDTree::predictTree(TreeNode *node, const MatrixXf testX)
{
    ...
    else
    {
        ...
        {
            //qDebug() << "attributeInterval.size(): " << attributeInterval.size() << "node->children.size(): " << node->children.size();
            //qWarning() << "分支节点的分支数和该节点的区间数不同";
            return predictTree(node->children[0], testX);
           // return 0;
          //  exit(0);
        }
        //TreeNode *newroot;
        ...
        if (! findflag)
        {
            qWarning()  << "预测取值不在该分支节点的任一一个区间内，程序应该是有问题了，";
            exit(0);
        }
...
MyNode<double>* CDTree::preOrder(CDTree::TreeNode *T)
{
    MyNode<double> *newnode = new MyNode<double>("", -1);
//    newnode->leftNode = zero;
//    newnode->rightNode = one;

    if (T->LeafNode){//如果二叉树为空直接返回结束程序，下同
        if(T->label == 1){
            return one;
        } else {
            return zero;
        ...
        newnode->feature_name = "feature " + QString::number(T->AttributeIndex).toStdString();
        double condition = intervals[T->AttributeIndex][0].first;
        qDebug() << "node num: " <<  T->AttributeIndex << "是否为叶子节点: " << T->LeafNode << "condition: "<< condition;  //先输出根节点的那个数字
        newnode->split_condition = condition;
        if (numbers.count(T->children[0]->AttributeIndex)==0) {
            numbers.insert(T->children[0]->AttributeIndex);
            newnode->leftNode = preOrder(T->children[0]);          //然后访问左孩子，开始递归
        } else {
            if (T->children[0]->LeafNode) newnode->leftNode = preOrder(T->children[0]);
        }
        if (numbers.count(T->children[1]->AttributeIndex)==0) {
            numbers.insert(T->children[1]->AttributeIndex);
            newnode->rightNode = preOrder(T->children[1]);          //左孩子访问完成，开始右孩子递归
        } else {
            if (T->children[1]->LeafNode) newnode->rightNode = preOrder(T->children[1]);
        }
        return newnode;
    } else {
        qDebug() << "node num: " <<  T->AttributeIndex << "是否为叶子节点: " << T->LeafNode << "condition: "<< intervals[T->AttributeIndex] << "说明不需要分，只有这个子节点";  //先输出根节点的那个数字
        newnode->split_condition = -1;//condition=-1  表示不需要分，此节点的属性值对结果没影响
        newnode->leftNode = preOrder(T->children[0]);          //然后访问左孩子，开始递归
